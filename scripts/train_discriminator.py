...
def _get_roc_auc_score(model, eval_dataloader, do_regression):
    """Measure performance of a model on the evaluation set.
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        probs_batch = soft(batch_logits)

        labels.extend(batch_labels.tolist())
        probs.extend(probs_batch[:, 1].tolist())
    model.train()
...
def train_model(args):
    ...
    if args.dataset_folder is not None:
        ...
        if not('dilma' in args.dataset_folder or 'sampling' in args.dataset_folder):
            data = pd.read_csv(args.dataset_folder)
            data = data[data['result_type'] == 'Successful']
            texts = [clean_text(i) for i in
                     clear_texts(data['original_text'].tolist()) + clear_texts(data['perturbed_text'].tolist())]
            labels = [0] * len(data['original_text'].tolist()) + \
                [1] * len(data['perturbed_text'].tolist())

            train_text, eval_text, train_labels, eval_labels = train_test_split(
                texts, labels, test_size=0.5, random_state=random.randint(1, 1e6), stratify=labels
            )

        else:
            data = load_jsonlines(args.dataset_folder)
            data = [i for i in data if i['data']['label']
                    != i['adversarial_data']['label']]
            texts = [clean_text(i['data']['text']) for i in data] + \
                [clean_text(i['adversarial_data']['text']) for i in data]
...
#     save_file_for_model_loading(args.output_dir, args.num_labels)
    ...
    for epoch in tqdm.trange(
        ...
        if (not adversarial_training) or (epoch >= args.num_clean_epochs):
            ...
            roc_auc_score_ = _get_roc_auc_score(
                model, eval_dataloader, args.do_regression)
            ...
            logger.info(
                f"Epoch {epoch} Eval {'pearson correlation' if args.do_regression else 'accuracy'}: {eval_score*100}% roc-auc_score {roc_auc_score_*100}"
            )
    ...
    roc_auc_score_ = _get_roc_auc_score(
        model, eval_dataloader, args.do_regression)
    logger.info(
        f"Saved model {'pearson correlation' if args.do_regression else 'accuracy'}: {eval_score*100}% roc-auc-score {roc_auc_score_*100}%"
    )
    metrics = {'accuracy': eval_score, 'roc_auc': roc_auc_score_}

    print(metrics)
    if args.metrics_output_path is None:
        json.dump(metrics, Path(args.dataset_folder, 'metrics.json').open('w'))
    else:
        json.dump(metrics, Path(args.metrics_output_path).open('w'))
