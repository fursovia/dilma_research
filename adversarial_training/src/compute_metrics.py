...
def metrics_file(file_path, before_or_after=None):
    ...
    if before_or_after is not None:
        before_or_after = 'after' if int(name.split('_')[1]) == 2 else 'before'
    ...
    original_text = [re.sub(r"(\[\[)|(\]\])", "", i)
                     for i in data['original_text'].tolist()]
    perturbed_text = [re.sub(r"(\[\[)|(\]\])", "", i)
                      for i in data['perturbed_text'].tolist()]
...
def metrics_folder(folder_path, save_path='metrics.csv'):
    ...
    for path in tqdm.tqdm(paths):
        if not str(path).endswith('3.csv'):
