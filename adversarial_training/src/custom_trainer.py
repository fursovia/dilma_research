"""
The same one as in Huggingface 3.3.0.
But saves model only if validation accuracy (after epoch end) is the best one.
Stops, if validation loss raises for several epochs.
"""
...
from transformers.trainer_utils import EvaluationStrategy, TrainOutput
...
class CustomTrainer(Trainer):
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial",
                                                                   Dict[str, Any]] = None, stat_file_for_saving: Optional[str] = None):
        """
        ...
        if self.model_init is not None:
            # Seed must be set before instantiating the model when using
            # model_init.
        ...
        if model_path is not None:
            # set global_step to global_step of last saved checkpoint from
            # model path
            try:
                self.global_step = int(model_path.split(
                    "-")[-1].split(os.path.sep)[0])
                self.total_flos = getattr(
                    self._actual_model(model).config, "total_flos", 0)
        ...
        validation_stat = {
            'accuracy': np.zeros(int(num_train_epochs)),
            'loss': np.zeros(int(num_train_epochs))
        }
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (self.args.logging_steps > 0 and self.global_step % self.args.logging_steps == 0) or (
                        ...
                        logs["epoch"] = int(
                            epoch) + np.round(step / len(train_dataloader), 3)
                    ...
                    if (
                        ...
                        validation_stat['accuracy'][epoch] = metrics['eval_accuracy']
                        validation_stat['loss'][epoch] = metrics['eval_loss']

                    if self.args.save_steps > 0 and self.global_step % self.args.save_steps == 0:
                        if validation_stat['accuracy'][:epoch + 1].argmax(
                        ) == epoch:
...
#                                 xm.save(
#                                     self.optimizer.state_dict(), os.path.join(
#                                         output_dir, "optimizer.pt"))
#                                 with warnings.catch_warnings(record=True) as caught_warnings:
#                                     xm.save(
#                                         self.lr_scheduler.state_dict(), os.path.join(
...
#                 self._report_to_hp_search(trial, epoch, metrics)
            ...
            if self.args.max_steps > 0 and self.global_step >= self.args.max_steps:
                break
            if epoch > 1:
                if validation_stat['loss'][epoch - 2:epoch + 1].argmax() == 2:
                    break
