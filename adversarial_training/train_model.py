...
class OtherArguments:
    ...
    adversarial_training_original_data_amount: int = field(
        metadata={"help": "number of non-adversarial sentences to use during adv. training"}, 
        default=1000,
    )
    adversarial_training_perturbed_data_amount: int = field(
        metadata={"help": "number of adversarial sentences to use during adv. training"}, 
        default=1000,
    )
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments, OtherArguments))
    model_args, data_args, training_args, other_args = parser.parse_args_into_dataclasses()
    training_args.disable_tqdm = True
    
    assert data_args.task_name in ['tomato', 'sst2', 'news', 'dstc'], 'Unknown dataset'
    ...
    if other_args.adversarial_data_path is None:
        ...
        train_data = get_data(f"datasets/{data_args.task_name}/data/train.txt")
        train_data = random.sample(train_data, min(len(train_data), other_args.adversarial_training_original_data_amount))
        adv_data = get_adv_data(other_args.adversarial_data_path)
        adv_data = random.sample(adv_data, min(len(adv_data), other_args.adversarial_training_perturbed_data_amount))
    ...
    test_data = get_data(f"datasets/{data_args.task_name}/data/{'validation' if data_args.task_name != 'news' else 'test'}.txt")
    ...
    config = AutoConfig.from_pretrained(model_args.config_name, num_labels = len(sorted(set(train_dataset.labels))))
    model = AutoModelForSequenceClassification.from_pretrained(model_args.model_name_or_path, config = config)
    

    batches_in_epoch = len(train_dataset) // training_args.per_device_train_batch_size + int(len(train_dataset)% training_args.per_device_train_batch_size > 0) 
