...
def save_dataset(dataset: Dict[str, List[Dict[str, Any]]], 
    ...
    Path(path, dataset_name, 'adv_trained_model').mkdir(parents=True, exist_ok=True)
    ...
    for model_type in ['model', 'adv_trained_model']:
        with open(f"{path}/{dataset_name}/{model_type}/load_{dataset_name}_model.py", "w") as file_handler:
            for item in load_model_file(dataset_name, model_type, len(set([i[label_name] for i in dataset[list(dataset.keys())[0]]]))):
                file_handler.write("{}\n".format(item))
...
def parse_arguments():
    ...
    parser.add_argument('--from_huggingface', action='store_true', default = None)
